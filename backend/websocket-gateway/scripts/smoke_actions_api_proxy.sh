...
HOST_ID="gateway-smoke-$(date +%Y%m%d%H%M%S)"
...
echo "Starting WebSocket Gateway and Actions API smoke stack..."
...
echo "Posting registration init through WebSocket Gateway for ${HOST_ID}..."
response="$(curl -fsS -X POST \
  -H "Content-Type: application/json" \
  -d "{\"org_id\":\"smoke-org\",\"host_id\":\"${HOST_ID}\",\"agent_pubkey\":\"${AGENT_PUBKEY}\",\"agent_version\":\"smoke\"}" \
  "${GATEWAY_BASE_URL}/agents/register/init")"
...
missing = [key for key in ("registration_id", "nonce", "server_time") if not response.get(key)]
if missing:
    raise SystemExit(f"registration init response missing {missing}: {response}")
print(json.dumps({"ok": True, "registration_id": response["registration_id"]}, sort_keys=True))
PY
