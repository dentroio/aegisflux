...
echo "Starting live agent receive smoke stack..."
...
echo "Connecting smoke agent and validating WebSocket delivery..."
(
  cd "${ROOT_DIR}/backend/websocket-gateway"
  ACTIONS_API_BASE_URL="${ACTIONS_API_BASE_URL}" \
  GATEWAY_BASE_URL="${GATEWAY_BASE_URL}" \
  go run ./cmd/smoke-agent-receive
)
