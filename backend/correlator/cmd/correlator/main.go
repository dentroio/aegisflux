...
func main() {
	natsURL := getenv("CORR_NATS_URL", getenv("NATS_URL", "nats://localhost:4222"))
	httpAddr := getenv("CORR_HTTP_ADDR", ":8085")
	...
	nc.Subscribe("etl.enriched", func(m *nats.Msg) {
		...
		sev := "info"
		if cs, ok := in["candidates"].([]any); ok && len(cs) > 0 {
			sev = "medium"
		}
		f := Finding{
	...
	log.Printf("[correlator] up on %s; NATS=%s", httpAddr, natsURL)
