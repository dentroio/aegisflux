# Windows dynamic detection packs (WO-DET-005)
...
| Method | Path | Purpose |
|--------|------|--------|
| GET | `/detection-packs/latest?os=windows&agent_version={semver}` | Discover latest compatible artifact metadata |
| GET | `/detection-packs/{pack_id}/artifact?os=windows&agent_version={semver}` | Download signed pack JSON |
...
3. **Schema / policy:** `schema_version`, `mode: observe`, `supported_os` includes `windows`, semver `sensor_version >= min_agent_version`, optional `expires_at`, `signature.algorithm == ed25519`, rule shape checks.
...
Under `AEGIS_DETECTION_PACK_CACHE` or, by default, `C:\ProgramData\Aegis\Agent\detection-pack\`:
