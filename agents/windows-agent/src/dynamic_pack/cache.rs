//! On-disk cache for verified active and previous packs (WO-DET-005).
...
pub fn default_cache_dir(spool: &Path) -> PathBuf {
    let parent = spool.parent().unwrap_or_else(|| Path::new("."));
    if parent.file_name().and_then(|name| name.to_str()) == Some("spool") {
        return parent.parent().unwrap_or(parent).join("detection-pack");
    }
    parent.join("detection-pack")
}
