...
/// Validate pack fields required for safe Linux evaluation.
pub fn validate_pack_for_linux(
    pack: &Value,
    agent_semver: &str,
    now: SystemTime,
) -> Result<(), String> {
    ...
    let has_linux = supported
        .iter()
        .filter_map(|x| x.as_str())
        .any(|s| s == "linux");
    if !has_linux {
        return Err("supported_os must include linux".to_string());
    }
    ...
    if alg != "ed25519" {
        return Err("only ed25519 signatures are supported on linux-agent".to_string());
    }
    ...
    for (i, rule) in rules.iter().enumerate() {
        ...
        if let Some(tos) = ro.get("target_os").and_then(|v| v.as_array()) {
            if !tos.iter().filter_map(|x| x.as_str()).any(|s| s == "linux") {
                return Err(format!(
                    "rule {} target_os excludes linux",
                    read_rule_id(ro)
                ));
            }
        }
...
mod tests {
    ...
    fn rejects_non_observe_mode() {
        ...
        let r = validate_pack_for_linux(&pack, "1.0.0", SystemTime::UNIX_EPOCH);
        assert!(r.is_err());
    }

    #[test]
    fn rejects_missing_linux() {
        let pack = serde_json::json!({
            "schema_version": "detection_pack.v1",
            "mode": "observe",
