...
use super::schema_check::validate_pack_for_linux;
use super::verify::{
...
pub fn run_dynamic_pack_pipeline(
    ...
    let Some(pk) = config.detection_pack_public_key else {
        eprintln!(
            "aegis-linux-agent: AEGIS_DETECTION_PACKS_ENABLED but no AEGIS_DETECTION_PACK_PUBLIC_KEY; skipping dynamic packs"
        );
        return Vec::new();
    };
    ...
    let active_before = match load_active(cache_dir) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("aegis-linux-agent: detection pack cache read: {e}");
            None
        }
    };

    let Some(controller) = config.controller_url.as_deref() else {
        eprintln!("aegis-linux-agent: AEGIS_CONTROLLER_URL unset; using verified cache only");
    ...
    let Ok(http) = HttpBase::parse(controller) else {
        eprintln!("aegis-linux-agent: invalid AEGIS_CONTROLLER_URL");
        let ctx = RolloutContext::stale("bad_controller_url", active_before.as_ref());
        return evaluate_cached_only(config, visibility_events, pk, cache_dir, &ctx);
    };

    let latest_path = format!(
        "/detection-packs/latest?os=linux&agent_version={}",
    ...
    let latest = match http.get(&latest_path) {
        Ok((200, _hdr, body)) => match serde_json::from_slice::<LatestResponse>(&body) {
            Ok(l) => Some(l),
            Err(e) => {
                eprintln!("aegis-linux-agent: latest pack json: {e}");
                None
            }
        },
        Ok((code, _, _)) => {
            eprintln!("aegis-linux-agent: GET latest returned HTTP {code}");
            None
        }
        Err(e) => {
            eprintln!("aegis-linux-agent: GET latest: {e}");
            None
        }
    };
    ...
    let pack_and_meta = if use_cached {
        ...
        match serde_json::from_slice::<Value>(&c.bytes) {
            Ok(v) => match verify_cached_pack(&v, &c.meta, &c.bytes, &pk, config) {
                ...
                Err(e) => {
                    eprintln!("aegis-linux-agent: cached pack no longer valid: {e}");
                    None
                }
            },
            Err(e) => {
                eprintln!("aegis-linux-agent: cached pack json: {e}");
                None
            }
        }
    } else {
        match http.get(&artifact_path) {
            Ok((200, hdr, body)) => match verify_downloaded(&body, &hdr, &pk, &latest, config) {
                Ok((v, meta)) => {
                    if let Err(e) = if let Some(prev) = active_before.as_ref() {
                        ...
                        eprintln!("aegis-linux-agent: cache write: {e}");
                    }
                ...
                Err(ctx) => {
                    let _ = post_status(config, &http, &ctx);
                    eprintln!(
                        "aegis-linux-agent: pack rejected: {}",
            ...
            Ok((code, _, _)) => {
                eprintln!("aegis-linux-agent: GET artifact HTTP {code}");
                let ctx =
            ...
            Err(e) => {
                eprintln!("aegis-linux-agent: GET artifact: {e}");
...
fn verify_cached_pack(
    ...
    validate_pack_for_linux(pack, &config.sensor_version, SystemTime::now())?;
...
fn evaluate_verified_pack(
    ...
    if let Err(e) = validate_pack_for_linux(pack, &config.sensor_version, SystemTime::now()) {
        eprintln!("aegis-linux-agent: pack policy check before eval: {e}");
        return Vec::new();
    }
    let pack_id = pack
        .get("pack_id")
    ...
    let matches = match evaluate_pack(pack, &batch) {
        Ok(m) => m,
        Err(e) => {
            eprintln!("aegis-linux-agent: pack evaluation: {e}");
            return Vec::new();
        }
    };
